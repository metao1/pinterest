...
import static com.metao.asyncdownloader.repository.request.RequestOptions.decodeTypeOf;
import static com.metao.asyncdownloader.repository.request.RequestOptions.diskCacheStrategyOf;
import static com.metao.asyncdownloader.repository.request.RequestOptions.skipMemoryCacheOf;
...
import com.metao.asyncdownloader.repository.load.resource.drawable.DrawableTransitionOptions;
...
import com.metao.asyncdownloader.repository.request.target.ViewTarget;
import com.metao.asyncdownloader.repository.request.transition.Transition;
import com.metao.asyncdownloader.repository.util.Synthetic;
import com.metao.asyncdownloader.repository.util.Util;
import java.io.File;

/**
 * A class for managing and starting requests for ImageDownloader. Can use activity, fragment and connectivity
 ...
 * handling, use the static ImageDownloader.load methods with your Fragment or Activity.
 *
 * @see ImageDownloader#with(android.app.Activity)
 * @see ImageDownloader#with(android.support.v4.app.FragmentActivity)
 * @see ImageDownloader#with(android.app.Fragment)
 * @see ImageDownloader#with(android.support.v4.app.Fragment)
 * @see ImageDownloader#with(Context)
 */
public class RequestManager implements LifecycleListener {
  ...
  public RequestManager(ImageDownloader imageDownloader, Lifecycle lifecycle, RequestManagerTreeNode treeNode) {
    this(imageDownloader, lifecycle, treeNode, new RequestTracker(), imageDownloader.getConnectivityMonitorFactory());
  }
  ...
  RequestManager(
    ...
    final Context context = imageDownloader.getImageDownloaderContext().getBaseContext();
    ...
    defaultRequestOptions = imageDownloader.getImageDownloaderContext().getDefaultRequestOptions();
    requestOptions = defaultRequestOptions;

    imageDownloader.registerRequestManager(this);
  }
   ...
   * ImageDownloaderBuilder#setDefaultRequestOptions(RequestOptions)}. If there are conflicts, the options
   * applied here will win. Note that this method does not mutate options provided to
   * {@link ImageDownloaderBuilder#setDefaultRequestOptions(RequestOptions)}.
   *
   ...
   * via {@link ImageDownloaderBuilder#setDefaultRequestOptions(RequestOptions)}, {@link
  ...
  public void onTrimMemory(int level) {
    imageDownloader.getImageDownloaderContext().onTrimMemory(level);
  }
  ...
  public void onLowMemory() {
    imageDownloader.getImageDownloaderContext().onLowMemory();
  }
  ...
  public void onDestroy() {
    ...
    imageDownloader.unregisterRequestManager(this);
  }
   ...
   * {@link com.metao.asyncdownloader.repository.imagehandler.load.resource.gif.GifDrawable}.
   *
   ...
   * com.metao.asyncdownloader.repository.imagehandler.load.ResourceDecoder}s that can decode any subclass of {@link Drawable}.
   *
   ...
   * com.metao.asyncdownloader.repository.imagehandler.load.engine.DiskCacheStrategy#DATA}. As a result, specifying a
   * {@link com.metao.asyncdownloader.repository.imagehandler.load.engine.DiskCacheStrategy} on this request is generally not
   * recommended.
   *
   ...
   * obtained from the media store (for local images/videos), or using ImageDownloader's disk cache
   ...
   * {@link com.metao.asyncdownloader.repository.imagehandler.load.ResourceDecoder}s
  ...
  public <ResourceType> RequestBuilder<ResourceType> as(Class<ResourceType> resourceClass) {
    return new RequestBuilder<>(imageDownloader.getImageDownloaderContext(), this, resourceClass);
  }

  /**
   * Cancel any pending loads ImageDownloader may have for the view and free any resources that may have been
   * loaded for the view.
   *
   * <p> Note that this will only work if {@link View#setTag(Object)} is not called on this view
   * outside of ImageDownloader. </p>
   *
   * @param view The view to cancel loads and free resources for.
   * @throws IllegalArgumentException if an object other than ImageDownloader's metadata is put as the view's
   *                                  tag.
   * @see #clear(Target)
   */
  ...
  /**
   * Cancel any pending loads ImageDownloader may have for the target and free any resources (such as
  ...
  private void untrackOrDelegate(Target<?> target) {
    ...
    if (!isOwnedByUs) {
      imageDownloader.removeFromManagers(target);
    }
  }
