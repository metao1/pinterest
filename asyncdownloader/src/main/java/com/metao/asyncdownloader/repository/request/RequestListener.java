...
public interface RequestListener<R> {
   ...
   * ImageDownloader.load()} to avoid object churn.
   *
   ...
   *     ImageDownloader.load(model).into(target);
   * }
   * }
   * </pre>
   * </p>
   ...
   *         {@code false} to allow ImageDownloader's request to update the target.
   */
  boolean onLoadFailed(@Nullable ImageDownloaderException e, Object model, Target<R> target,
                       boolean isFirstResource);

  /**
   * Called when a load completes successfully, immediately after {@link
   * Target#onResourceReady(Object, com.metao.asyncdownloader.repository.request.transition.Transition)}.
   *
