...
/**
 * For a given {@link com.metao.asyncdownloader.repository.load.data.DataFetcher} for a given data class, attempts to
 * fetch the data and then run it through one or more
 * {@link com.metao.asyncdownloader.repository.load.engine.DecodePath}s.
 *
...
public class LoadPath<Data, ResourceType, Transcode> {
  ...
  public Resource<Transcode> load(DataRewinder<Data> rewinder, Options options, int width,
      int height, DecodePath.DecodeCallback<ResourceType> decodeCallback) throws ImageDownloaderException {
  ...
  private Resource<Transcode> loadWithExceptionList(DataRewinder<Data> rewinder, Options options,
      ...
      List<Exception> exceptions) throws ImageDownloaderException {
    ...
    if (result == null) {
      throw new ImageDownloaderException(failureMessage, new ArrayList<>(exceptions));
    }
