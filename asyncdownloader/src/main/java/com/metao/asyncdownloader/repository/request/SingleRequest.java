...
import com.metao.asyncdownloader.repository.core.ImageDownloaderContext;
...
import com.metao.asyncdownloader.repository.request.target.SizeReadyCallback;
...
import com.metao.asyncdownloader.repository.request.transition.TransitionFactory;
...
/**
 * A {@link Request} that loads a {@link com.metao.asyncdownloader.repository.load.engine.Resource} into a given
 * {@link Target}.
 *
...
public final class SingleRequest<R> implements Request,
  ...
  private static final String ImageDownloader_TAG = "ImageDownloader";
  ...
  private ImageDownloaderContext ImageDownloaderContext;
  ...
  public static <R> SingleRequest<R> obtain(
      ImageDownloaderContext ImageDownloaderContext,
      Object model,
  ...
  private void init(
      ...
      Engine engine,
      TransitionFactory<? super R> animationFactory) {
    this.ImageDownloaderContext = ImageDownloaderContext;
    this.model = model;
  ...
  public void begin() {
    ...
    if (model == null) {
      ...
      onLoadFailed(new ImageDownloaderException("Received null model"), logLevel);
      return;
    }
  ...
  private Drawable loadDrawable(int resourceId) {
    Resources resources = ImageDownloaderContext.getResources();
  ...
  public void onResourceReady(Resource<?> resource, DataSource dataSource) {
    ...
    loadStatus = null;
    if (resource == null) {
      ImageDownloaderException exception = new ImageDownloaderException("Expected to receive a Resource<R> with an "
    ...
    if (received == null || !transcodeClass.isAssignableFrom(received.getClass())) {
      releaseResource(resource);
      ImageDownloaderException exception = new ImageDownloaderException("Expected to receive an object of "
  ...
  private void onResourceReady(Resource<R> resource, R result, DataSource dataSource) {
    ...
    if (ImageDownloaderContext.getLogLevel() <= Log.DEBUG) {
      Log.d(ImageDownloader_TAG, "Finished loading " + result.getClass().getSimpleName() + " from "
   ...
   */
  @Override
  public void onLoadFailed(ImageDownloaderException e) {
    onLoadFailed(e, Log.WARN);
  }

  private void onLoadFailed(ImageDownloaderException e, int maxLogLevel) {
    stateVerifier.throwIfRecycled();
    int logLevel = ImageDownloaderContext.getLogLevel();
    if (logLevel <= maxLogLevel) {
      Log.w(ImageDownloader_TAG, "Load failed for " + model + " with size [" + width + "x" + height + "]", e);
      if (logLevel <= Log.INFO) {
        e.logRootCauses(ImageDownloader_TAG);
      }
    }
