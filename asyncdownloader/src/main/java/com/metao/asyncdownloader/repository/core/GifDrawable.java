...
public class GifDrawable extends Drawable implements GifFrameLoader.FrameCallback,
   ...
   * @param bitmapPool          A {@link com.metao.asyncdownloader.repository.load.engine.bitmap_recycle.BitmapPool}
   *                            that can be used to return the first frame when this drawable is
   *                            recycled.
   * @param frameTransformation An {@link com.metao.asyncdownloader.repository.load.Transformation} that can be
   *                            applied to each frame.
   * @param targetFrameWidth    The desired width of the frames displayed by this drawable (the
   *                            width of the view or
   *                            {@link com.metao.asyncdownloader.repository.request.target.Target}
   ...
   * @see #setFrameTransformation(com.metao.asyncdownloader.repository.load.Transformation, Bitmap)
   */
  public GifDrawable(Context context, GifDecoder gifDecoder, BitmapPool bitmapPool,
    ...
    this(
        new GifState(
            bitmapPool,
            new GifFrameLoader(
                // TODO(b/27524013): Factor out this call to ImageDownloader.get()
                ImageDownloader.get(context),
                gifDecoder,
                targetFrameWidth,
                targetFrameHeight,
                frameTransformation,
  ...
  public void setLoopCount(int loopCount) {
    if (loopCount <= 0 && loopCount != LOOP_FOREVER && loopCount != LOOP_INTRINSIC) {
      throw new IllegalArgumentException("Loop count must be greater than 0, or equal to "
          + "ImageDownloaderDrawable.LOOP_FOREVER, or equal to ImageDownloaderDrawable.LOOP_INTRINSIC");
    }
