...
import com.metao.asyncdownloader.repository.util.pool.FactoryPools.Poolable;
...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback,
  ...
  private void notifyFailed() {
    setNotifiedOrThrow();
    ImageDownloaderException e = new ImageDownloaderException("Failed to load resource", new ArrayList<>(exceptions));
  ...
  public void onDataFetcherFailed(Key attemptedKey, Exception e, DataFetcher<?> fetcher,
    ...
    fetcher.cleanup();
    ImageDownloaderException exception = new ImageDownloaderException("Fetching data failed", e);
  ...
  private <Data> Resource<R> decodeFromData(DataFetcher<?> fetcher, Data data,
      DataSource dataSource) throws ImageDownloaderException {
    try {
      if (data == null) {
        return null;
      }
  ...
  private <Data, ResourceType> Resource<R> runLoadPath(Data data, DataSource dataSource,
      LoadPath<Data, ResourceType, R> path) throws ImageDownloaderException {
    DataRewinder<Data> rewinder = ImageDownloaderContext.getRegistry().getRewinder(data);
    try {
