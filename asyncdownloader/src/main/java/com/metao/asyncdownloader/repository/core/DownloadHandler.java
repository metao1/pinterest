...
import com.metao.asyncdownloader.download.core.AsyncDownloadHandler;
...
public class DownloadHandler<T> {
    ...
    protected final void setRepoCallback(String taskId, RepositoryCallback<T> repoCallback) {
        callbacks.put(taskId, repoCallback);
    }

    private synchronized void dispatchDownloadSignal(String taskId, T o) {
    ...
    private synchronized void dispatchProgressSignal(String taskId, Object object) {
        Enumeration<String> keys = callbacks.keys();
        while (keys.hasMoreElements()) {
            String key = keys.nextElement();
