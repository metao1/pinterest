...
import com.metao.asyncdownloader.repository.load.model.Headers;
 ...
 */
public abstract class BaseImageDownloaderUrlLoader<Model> implements ModelLoader<Model, InputStream> {
  private final ModelLoader<ImageDownloaderUrl, InputStream> concreteLoader;
  @Nullable private final ModelCache<Model, ImageDownloaderUrl> modelCache;

  protected BaseImageDownloaderUrlLoader(ModelLoader<ImageDownloaderUrl, InputStream> concreteLoader) {
    this(concreteLoader, null);
  }

  protected BaseImageDownloaderUrlLoader(ModelLoader<ImageDownloaderUrl, InputStream> concreteLoader,
      @Nullable ModelCache<Model, ImageDownloaderUrl> modelCache) {
  ...
  public LoadData<InputStream> buildLoadData(Model model, int width, int height, Options options) {
    ImageDownloaderUrl result = null;
    if (modelCache != null) {
    ...
    if (result == null) {
      ...
      result = new ImageDownloaderUrl(stringURL, getHeaders(model, width, height, options));
  ...
  private static List<Key> getAlternateKeys(List<String> alternateUrls) {
    ...
    for (String alternate : alternateUrls) {
      result.add(new ImageDownloaderUrl(alternate));
    }
    return result;
  }
   ...
   * as the primary url.
   *
   * <p> Implementing this method allows ImageDownloader to fulfill requests for bucketed images in smaller
