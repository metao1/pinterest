...
import com.metao.asyncdownloader.repository.load.model.ModelLoader.LoadData;
...
final class DecodeHelper<Transcode> {
  ...
  List<Class<?>> getRegisteredResourceClasses() {
    return ImageDownloaderContext.getRegistry()
  ...
  <Data> LoadPath<Data, ?, Transcode> getLoadPath(Class<Data> dataClass) {
    return ImageDownloaderContext.getRegistry().getLoadPath(dataClass, resourceClass, transcodeClass);
  }
  ...
  boolean isResourceEncoderAvailable(Resource<?> resource) {
    return ImageDownloaderContext.getRegistry().isResourceEncoderAvailable(resource);
  }

  <Z> ResourceEncoder<Z> getResultEncoder(Resource<Z> resource) {
    return ImageDownloaderContext.getRegistry().getResultEncoder(resource);
  }

  List<ModelLoader<File, ?>> getModelLoaders(File file)
      throws Registry.NoModelLoaderAvailableException {
    return ImageDownloaderContext.getRegistry().getModelLoaders(file);
  }
  ...
  List<LoadData<?>> getLoadData() {
    if (!isLoadDataSet) {
      isLoadDataSet = true;
      loadData.clear();
      List<ModelLoader<Object, ?>> modelLoaders = ImageDownloaderContext.getRegistry().getModelLoaders(model);
  ...
  <X> Encoder<X> getSourceEncoder(X data) throws Registry.NoSourceEncoderAvailableException {
    return ImageDownloaderContext.getRegistry().getSourceEncoder(data);
  }
}
